...
- **62 文件 / 281 个测试全绿**，纯 TDD 完成，各计划均经两阶段 subagent 评审 + 终审后合入 main（清单见"下一步"）。
...
| `grade/{churn,coupling,ownership,centrality}.ts` | 四个信号（0..1，归一化）（中心度已分词化——建表 O(总字符数)+查表 Map.get,2026-07-13;非词字符名回退旧正则是仅存慢路径。2026-07-14 泛用名截断:df > max(⌈5%N⌉,20) 的名字贡献归零,`genericDfCutoff` 导出）|
