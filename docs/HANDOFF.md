...
8. ~~**vitest 突变探针**~~ ✅ 已完成（分支 feat/vitest-probe，见 `docs/superpowers/plans/2026-07-13-vitest-probe.md`）。verify 全语言闭环：runnerFor 分发 vitest（js/vue 共用一个 runner）；圈定=basename 索引镜像+引用扫描；位点=JS 语句+vue carve 区段（半行字节一致性守卫）；死码清理（未知语言→null）；vitest 逐文件隔离使加载失败=正常爆炸半径，judge 零改动。
