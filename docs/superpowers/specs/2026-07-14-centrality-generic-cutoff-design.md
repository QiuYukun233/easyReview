...
1. 先建 df 表:词名对 tokenCounts 新增一趟 O(唯一词名×文件数) 遍历建 df 表(chatwoot 规模实测中心度约 0.6s→1.2s,仍在秒级验收线内);非词名按**唯一名字**做一趟正则扫描同时得 df 与逐文件 occ(顺带修掉现状「同名字多定义文件重复全库扫描」的小浪费,行为不变)。
...
- 不修尾缀 ?/! 名字的 \b 怪癖(`\bvalid\?\b` 要求 ? 后紧跟词字符,真实 ruby 调用点如 `valid? &&` 匹配不上)——该怪癖新旧实现一致,意味着这类名字的扇入一直被系统性低估;属 v1 文本近似的固有局限,中心度 v2(引用图)一并解决。
