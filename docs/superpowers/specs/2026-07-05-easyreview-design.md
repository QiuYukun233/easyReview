...
> **修订 2026-07-09**:铁律第二句放宽——LLM 除标签外,允许生成**解读层**(块级综述 + 函数逐条,见 `2026-07-09-ai-interpretation-design.md`),前提四条:①喂料 = 确定性事实 + 真实源码,只描述给定内容、不发明结构;②默认开、一键可关;③解读永不参与「已理解/已验证」判定——验证仍只认 ground truth;④结构性判断(两轴、聚块、路径)仍 100% 确定性信号,一字不动。§13 非目标随之收窄。修订理由:2026-07-08 实际使用反馈,看不到解释比「解释可能退化」更拖速度(加速器不是减速带);退化风险用接地喂料 + 可关 + 不参与判定三条围栏对冲。§2/§13 正文已按此修订,原文见 git 历史。
...
一句话铁律：**所有结构性判断（两个轴、叶子如何聚成块）都由确定性 VCS/静态/执行信号算出；LLM 贴标签、生成接地解读（喂确定性事实+真实源码，默认开、一键可关、永不参与"已理解/已验证"判定——见修订 2026-07-09），绝不发明结构。理解靠撞运行时 ground truth 验证，不靠叙述。**
...
- 不做无接地的叙述式"全仓摘要"（注定退化成摘要）；接地、按需、可关的块级解读是做的（修订 2026-07-09，见 `2026-07-09-ai-interpretation-design.md`）。
- 不做多用户/团队/账号/云端。
- 不做多语言（仅 Rust 适配器）。
