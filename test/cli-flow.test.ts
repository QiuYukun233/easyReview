...
describe('runFlowProbe(编排:校验→沙箱→斩→单例跑→判定→报告)', () => {
  ...
  it('回退路径:方法定义行对不上 → chooseMutation 文件级,报告标注回退且突变用语言感知前缀', async () => {
    const repo = mkdtempSync(join(tmpdir(), 'er-probe-fb-'));
    const out = mkdtempSync(join(tmpdir(), 'er-probe-fb-out-'));
    writeFileSync(join(repo, 'easyreview.runner.json'), JSON.stringify({
      version: 1, ruby: { cmd: ['fake-rspec', '{specFiles}'] },
    }));
    mkdirSync(join(repo, 'app', 'models'), { recursive: true });
    // 方法体内有可注释语句(loner_call),但 rawTrace 的 line 指向非 def 行(line 3),
    // 令 pickSiteInMethods 的方法节点匹配 bail(精确/范围都对不上 method='save_it') → 回退文件级。
    writeFileSync(join(repo, 'app', 'models', 'm.rb'), [
      'class M',
      '  def save_it',
      '    loner_call(9)',
    ...
    writeFileSync(join(out, 'easyreview.flows.json'), JSON.stringify({
      ...
      flows: [{
        id: 'flow-m-L25', name: '单例流程',
        source: { kind: 'rspec-trace', spec: 'spec/m_spec.rb:25', tracedAt: '2026-07-16T00:00:00Z' },
        steps: [{ chunkId: 'app/models/m.rb', methods: ['ghost_method'], hits: 1, phase: 'request' }],
        rawTrace: [{ file: '/app/app/models/m.rb', method: 'ghost_method', line: 99 }],
      }],
    }));
    let mutatedSeen = '';
    const exec: Exec = async (_c, _a, cwd) => {
